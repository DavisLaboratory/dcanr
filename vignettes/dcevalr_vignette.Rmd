...
methods therefore data simulated from dynamical systems models is included. This
data has been previously simulated and the results are made available here.
Specific details on the simulation method and framework can be found in PAPER.
...
```{r installationBio}
#devtools::install_github('http://github.com/bhuva.d/dcevalr')
library(dcevalr)
```
...
## Step 0: Load an example dataset (simulated)
...
```{r message=FALSE, warning=FALSE, fig.small=TRUE}
library(igraph)
